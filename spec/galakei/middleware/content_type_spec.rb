...
describe Galakei::Middleware::ContentType do
  describe "from Docomo SH-06A" do
    before { @env = env_for("Docomo SH-06A") }
    describe "content type is text/html" do
      before { @app = Galakei::Middleware::ContentType.new(mock("app", :call => [ :status, {"Content-Type" => "text/html"}, :response ])) }
    ...
    describe "content type is text/html; charset=utf-8" do
      before { @app = Galakei::Middleware::ContentType.new(mock("app", :call => [ :status, {"Content-Type" => "text/html; charset=utf-8"}, :response ])) }
    ...
    describe "content type is image/png" do
      before { @app = Galakei::Middleware::ContentType.new(mock("app", :call => [ :status, {"Content-Type" => "image/png"}, :response ])) }
      it("should not change content type") do
        @app.call(@env)[1].should == {"Content-Type"=>"image/png"}
      end
    end
    describe "304 response" do
      before { @app = Galakei::Middleware::ContentType.new(mock("app", :call => [ 304, {}, :response ])) }
      it("should not change content type") do
